 ...
 * Notification sent when an organization is verified by an admin
 *
 * @param name - Organization name
 * @param dashboardUrl - URL to the organization dashboard
 * @param translations - Translation function
 * @param locale - Locale language
 * @param config - Platform config
 * @returns HTML string for email body
 */
export function getOrganizationApprovedTemplate(
    name: string,
    dashboardUrl: string,
    translations: any,
    locale: Locale = 'en',
    ...
    return `
        ...
        <head>
            ...
            <title>${t('email.orgApproved.subject')}</title>
        </head>
                            ...
                            <tr>
                                <td style="padding: 40px 30px;">
                                    <h2 style="color: ${lightColors.textPrimary}; font-size: 24px; font-weight: 600; margin: 0 0 16px 0; line-height: 1.3;">
                                        ${t('email.orgApproved.greeting')} ${name}!
                                    </h2>
                                    <p style="color: ${lightColors.textPrimary}; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
                                        ${t('email.orgApproved.body')}
                                    </p>
                                    <p style="color: ${lightColors.textPrimary}; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
                                        ${t('email.orgApproved.nextSteps')}
                                    </p>
                                            ...
                                            <td style="text-align: center;">
                                                <a href="${dashboardUrl}" 
                                                   style="display: inline-block; background-color: ${lightColors.primary}; color: ${lightColors.primaryForeground}; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: 500; font-size: 16px;">
                                                    ${t('email.orgApproved.buttonText')}
                                                </a>
                                            </td>
                                        </tr>
                                    </table>
