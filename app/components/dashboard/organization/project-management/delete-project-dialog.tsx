...
import { deleteMyOrganizationProject } from "@/lib/controller/organization/organization-projects-controller";
...
export function DeleteProjectDialog({ target, onOpenChange, onSuccess }: Props) {
  const t = useTranslations("organization.projects.table");
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await deleteMyOrganizationProject(target.id);
      if (result.success) {
