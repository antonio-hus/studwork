/** @format */
import 'server-only';
import type {ProjectCompletion, ProjectCompletionCreateType, ProjectCompletionUpdateType} from '@/lib/domain/project-completion';
...
export class ProjectCompletionService {
     ...
     */
    async getProjectCompletionById(id: string): Promise<ProjectCompletion | null> {
     ...
     */
    async createProjectCompletion(data: ProjectCompletionCreateType): Promise<ProjectCompletion> {
        try {
     ...
     */
    async updateProjectCompletion(id: string, data: ProjectCompletionUpdateType): Promise<ProjectCompletion> {
        try {
            return await ProjectCompletionRepository.instance.update(id, data);
        } catch (error) {
